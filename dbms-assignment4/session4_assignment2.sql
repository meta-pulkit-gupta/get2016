 ...
 * all issue details of the member using a simple SELECT command */
 CREATE VIEW member_issue_details AS
    SELECT m.member_name, bi.issue_date, bi.accession_number,
        bi.member_id, bi.due_date FROM book_issue bi
        INNER JOIN members m ON bi.member_id = m.member_id;
        
/* Using the above view to display issue details of a member */
SELECT member_name, issue_date, due_date, accession_number
...
SELECT subject_name, title_name, member_name, category, issue_date,
    due_date, return_date FROM information_view;